...
class Asset:
    ...
    def updateGuid(self, new_guid: Optional[str] = None):
        # ディレクトリ名を変更
        if self.deleted:
            return
        if new_guid is None:
            new_guid = hashlib.md5(self.guid.encode('ascii')).hexdigest()
        new_path = os.path.join(self.root_dpath, "../", new_guid)
        os.rename(self.root_dpath, new_path)

        # 変数群を変更
        self.__guid = new_guid
        self.__root_dpath = new_path
